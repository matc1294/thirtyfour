...
impl From<BySelector> for Selector {
    fn from(by: BySelector) -> Self {
        match by {
            BySelector::Id(x) => Selector::new("css selector", format!(r#"[id="{x}"]"#)),
            ...
            BySelector::Name(x) => Selector::new("css selector", format!(r#"[name="{x}"]"#)),
            BySelector::Tag(x) | BySelector::Css(x) => Selector::new("css selector", x),
            BySelector::ClassName(x) => Selector::new("css selector", format!(".{x}")),
            BySelector::Testid(x) => {
                Selector::new("testid selector", format!(r#"[data-testid="{x}"]"#))
            }
        }
    }
}
